...
public class Registration extends TransportUriGuarantee {
	 ...
	 */
	public static void registrationFinish(@Recaptcha(message="错误的验证码") String captcha,
			@Required(message="请填写您的电子邮件地址") @Email(message="错误的邮件地址") String email,
			@Required(message="请填写您希望使用的用户名") String username,
			@Required @Equals(message="两次输入的密码不一致",value="confirm") @MinSize(message="密码位数必须大于8位",value=8) String password,
			@Required @MinSize(message="密码位数必须大于8位",value=8) String confirm,
			@IsTrue(message="必须接受服务条款") Boolean acceptTermsOfService) {
		...
		if (!hasErrors) {
			...
			if (Casino.doesUserExist(email)) {
				hasErrors = true;
				validation.addError("email", "该邮箱已被注册", "");
			}
		}
		...
		if (hasErrors) {
			...
			Casino.createNewCasinoUser(email, username, passwordHash,
					confirmationCode);
	...
	public static void confirm(String code) {
		...
		if (email != null) {
        	...
        	session.put("fullname", currentUser.username);
